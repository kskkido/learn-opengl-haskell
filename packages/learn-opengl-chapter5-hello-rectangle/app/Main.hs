...
main = do
  ...
  let vertices = Vector.Storable.fromList
      ...
      indices = Vector.Storable.fromList
        ( [ 0, 1, 3
          , 1, 2, 3
          ] :: [Word32]
        )
  ...
  Program.exec_ do
    ...
    ebo <- Program.fromCallback Alloc.alloca
    liftIO do
      ...
      Vector.Storable.unsafeWith vertices $ \pointer ->
        GL.glBufferData
          GL.GL_ARRAY_BUFFER
      ...
      Foreign.peek ebo >>= GL.glBindBuffer GL.GL_ELEMENT_ARRAY_BUFFER
      Vector.Storable.unsafeWith indices $ \pointer ->
        GL.glBufferData
          GL.GL_ELEMENT_ARRAY_BUFFER
          (fromIntegral (Vector.Storable.length indices * Foreign.sizeOf (undefined :: Word32)))
          (Foreign.castPtr pointer)
          GL.GL_STATIC_DRAW
      Loops.whileM_ (not <$> GLFW.windowShouldClose window) do
        ...
        GL.glDrawElements
          GL.GL_TRIANGLES
          (fromInteger . toInteger . Vector.Storable.length $ indices)
          GL.GL_UNSIGNED_INT
          Foreign.nullPtr
      forM_ [fragmentShader, vertexShader] \shader -> do
